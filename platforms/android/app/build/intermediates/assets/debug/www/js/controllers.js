...
.controller('QuizzesCtrl', function($scope, $rootScope, Scores, $state) {
  ...
  $scope.$on('$ionicView.enter', function(e) {
    ...
    if ($rootScope.operationSelected == 'Addition') {
      ...
      result = $scope.numOne * $scope.numTwo;
      $scope.operate = '*';
    } else if ($rootScope.operationSelected == 'Remainder'){
      result = $scope.numOne % $scope.numTwo;
      $scope.operate = '%';
    }
    $scope.guesses = 0;
    ...
    $scope.checkAnswer = function() {
      ...
      if ($scope.guesses >= 10) {
        $scope.newscore.score = (result == $scope.inputAnswer ? 1 + $scope.correctAns : $scope.correctAns) + ' / 10';
